...
public class LSPatchProxyApplication extends Application {
    ...
    @Override protected void attachBaseContext(Context base) {
        ...
        if (realApplication != null) {
            try {
                ...
                attach.invoke(realApplication, base);
                replaceApplicationInstance(realApplication);
                Log.i(TAG, "Deep transparent delegation completed");
            } catch (Throwable t) { Log.e(TAG, "Deep delegation failed", t); }
        }
    }
    ...
    private void instantiateRealApplication(Context context) {
        try {
            ApplicationInfo ai = Build.VERSION.SDK_INT >= 33
                ? context.getPackageManager().getApplicationInfo(context.getPackageName(), PackageManager.ApplicationInfoFlags.of(PackageManager.GET_META_DATA))
                : context.getPackageManager().getApplicationInfo(context.getPackageName(), PackageManager.GET_META_DATA);
            if (ai.metaData != null && ai.metaData.containsKey(META_ORIG_APP)) {
                String cls = ai.metaData.getString(META_ORIG_APP);
                realApplication = (Application) context.getClassLoader().loadClass(cls).getDeclaredConstructor().newInstance();
                Log.i(TAG, "Delegated to real Application: " + cls);
            }
        } catch (Throwable t) { Log.w(TAG, "No real Application", t); }
    }
    private void replaceApplicationInstance(Application realApp) throws Exception {
        ...
        Field infoF = Class.forName("android.app.ActivityThread$AppBindData").getDeclaredField("info"); infoF.setAccessible(true);
        Object loadedApk = infoF.get(bound);
        Field mAppF = Class.forName("android.app.LoadedApk").getDeclaredField("mApplication"); mAppF.setAccessible(true);
        ...
        @SuppressWarnings("unchecked") ArrayList<Application> list = (ArrayList<Application>) mAll.get(at);
        if (list != null) { list.remove(this); if (!list.contains(realApp)) list.add(realApp); }
            ...
            for (Object rec : ((Map<?, ?>) map).values()) {
                try {
                    ...
                    if (prov != null) {
                        ...
                        ctxF.setAccessible(true); ctxF.set(prov, realApp);
                    }
